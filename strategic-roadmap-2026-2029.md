...
But culture alone cannot carry us through the next phase. We've outgrown our operating system. The people are capable — the machinery around them hasn't kept pace. Every surge in volume amplifies that gap. Error rates climb under pressure. Rework steals capacity. Engineering becomes a bottleneck when it should be an accelerator.

The next chapter is not about winning more work. We're already doing that. It's about building an engine room capable of delivering on what we're winning — consistently, at scale, without burning our people out.

---

## Our Strategic Framework

We operate against three Strategic Objectives:

| Objective | What it means |
|---|---|
| **1. Hunting on the Front Foot** | Proactive, targeted business development — owning the pipeline |
| **2. First Pass Business** | Touch it once. Robust engineering, zero rework, no surprises at delivery |
| **3. Increase Throughput** | More output per person, per dollar, per week — without adding proportional headcount |

These three are not independent. You cannot sustainably increase throughput if you're burning capacity on rework. You cannot hunt on the front foot if you can't reliably deliver what you're selling. The real leverage point is **Objective 2 — First Pass Business**. Fix that, and 1 and 3 both improve.

---

## Honest Assessment — Where We Stand Today

### What's Working
- **Culture** — our single greatest competitive advantage. People care, standards are held, and the team has navigated extraordinary growth with integrity
...
```
Engineering late and releasing incomplete packages
            ↓
Fabrication cannot close on time
            ↓
Operations costing and allocation cannot keep up
            ↓
Pressure floods the organisation → error rates climb → rework → repeat
```

This is not a people problem. It is a systems and process problem. The constraint is Engineering — and it has three specific root causes.
...
By June 2029 we should be able to answer yes to all of these:
...
|---|---|---|
| **Hunting on the Front Foot** | Qualified pipeline value ($) | Increase QoQ |
| | Quote-to-order conversion rate (%) | Baseline + improve |
